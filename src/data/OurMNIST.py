...
class TestMNIST(Dataset):
    ...
    def load_data(self, file_path, train):
        ...
        path = os.path.join(file_path, 'test')
    ...
    def create_env(self, data, bias_ratio, start_ratio, end_ratio):
        ...
        for cur_names, cur_images in zip(data.keys(), data.values()):

            names.append(cur_names)
            cur_images = torch.unsqueeze(cur_images, dim=0)
    ...
    def __getitem__(self, keys):
        ...
        for key in keys:
            env_id = int(key[1])
            idx.append(key[0])
            names.append(self.envs[env_id]['names'][key[0]])
